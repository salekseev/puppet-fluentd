...
#
# Parameters:
#  the name of this ressource reflects the either the filename of the plugin,
#  which must be copied to fluentd/files or the name of the gem
#
...
define fluentd::install_plugin (
  ...
  case $plugin_type {
    'file': {
      fluentd::install_plugin::file { [$plugin_name]:
        ensure  => $ensure,
        require => Class['Fluentd::Package']
      }
    }
    'gem': {
      fluentd::install_plugin::gem { [$plugin_name]:
        ensure  => $ensure,
        require => Class['Fluentd::Package']
      }
    }
    default: {
      fail("plugin_type => ${plugin_type} is currently not supported.")
    }
  }
}
